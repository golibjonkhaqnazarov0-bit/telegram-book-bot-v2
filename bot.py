...
async def admin_delete_book(callback: types.CallbackQuery):
    ...
    await callback.answer(f"✅ {book[0]} kitobi o'chirildi!", show_alert=True)
...
async def admin_announcement(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        return
...
async def admin_msg_user(callback: types.CallbackQuery):
    if callback.from_user.id != ADMIN_ID:
        return
...
async def admin_send_message_process(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
